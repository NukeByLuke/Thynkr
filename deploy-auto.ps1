...
Write-Host "Deploying to $dropletIp..." -ForegroundColor Cyan

# Create temp script file with Unix line endings
$scriptContent = @"
cd /root
cat >> .env << 'STRIPE_EOF'
...
$scriptContent | Out-File -FilePath "deploy-temp.sh" -Encoding ASCII -NoNewline
(Get-Content "deploy-temp.sh" -Raw) -replace "`r`n","`n" | Set-Content "deploy-temp.sh" -NoNewline

# Copy to server and execute
scp deploy-temp.sh root@${dropletIp}:/root/deploy-stripe.sh
ssh root@$dropletIp "bash /root/deploy-stripe.sh"
