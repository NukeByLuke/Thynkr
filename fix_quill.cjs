...
let handleQuillChangeStr = `  const handleQuillChange = (content: string) => {\n    setLocalNotes(content);\n    localStorage.setItem('my-notes-' + file.id, content);\n  };\n`;
if (!content.includes("handleQuillChange")) {
  content = content.replace("const handleNoteChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {", handleQuillChangeStr + "\n  const handleNoteChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {");
}
...
content = content.replace(/<textarea\s+className="flex-1 w-full h-\[50vh\].*?value=\{localNotes\}[\s\S]*?\/>/, newQuillEditor);
