        ...
        <div className="relative z-10 flex flex-col h-full p-16 justify-between">
          ...
          <div className="flex items-center gap-10">
            ...
            <div className="relative group flex-shrink-0">
               ...
               <div className="relative w-36 h-36 rounded-full border-[3px] border-slate-800/50 overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center shadow-2xl">
                 {user.avatarUrl ? (
                   <img src={user.avatarUrl} alt={user.username} className="w-full h-full object-cover" crossOrigin="anonymous" />
                 ) : (
                   <span className="text-6xl font-bold bg-clip-text text-transparent bg-gradient-to-br from-slate-300 to-slate-500">{user.username.charAt(0).toUpperCase()}</span>
                 )}
               </div>
               
               {/* Enhanced Level Badge */}
               <div className="absolute -bottom-2 -right-2 bg-slate-950 p-1.5 rounded-full shadow-xl">
                 <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-orange-600 text-white font-bold px-5 py-2 rounded-full border-2 border-slate-700/50 shadow-lg flex items-center gap-1.5">
                   <span className="text-amber-100 text-base">LEVEL {user.level}</span>
                 </div>
               </div>
            </div>
            ...
            <div className="flex flex-col flex-1 gap-3">
              <h1 className="text-6xl font-extrabold mb-1 bg-clip-text text-transparent bg-gradient-to-r from-white via-slate-200 to-slate-400 tracking-tight drop-shadow-lg leading-tight">
                {user.username}
              </h1>
              <p className="text-slate-400 font-semibold text-xl flex items-center gap-2 mb-3">
                Thynkr Scholar
              </p>

              {/* Enhanced Badges with Better Styling */}
              <div className="flex items-center gap-5">
                <div className="px-6 py-3 rounded-xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-slate-700/60 flex items-center gap-3 backdrop-blur-md shadow-lg">
                ...
                <div className="px-6 py-3 rounded-xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-slate-700/60 flex items-center gap-3 backdrop-blur-md shadow-lg">
                   <Award size={22} className="text-purple-400 drop-shadow-[0_0_8px_rgba(192,132,252,0.5)]" />
          ...
          {featuredStats && featuredStats.length > 0 && (
            <div className="grid grid-cols-3 gap-8 border-t border-slate-800/60 pt-10 mt-6">
              {featuredStats.map((stat, idx) => (
                <div key={idx} className="flex flex-col gap-3 group">
                  ...
                  <span className="text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-br from-white to-slate-300">{stat.value}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="absolute bottom-8 right-10 flex items-center gap-3 opacity-70 hover:opacity-100 transition-opacity">
            <div className="w-2.5 h-2.5 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 shadow-[0_0_12px_rgba(59,130,246,0.8)] animate-pulse" style={{ animationDuration: '2s' }} />
            <span className="text-sm font-bold tracking-[0.25em] text-slate-300 uppercase">
