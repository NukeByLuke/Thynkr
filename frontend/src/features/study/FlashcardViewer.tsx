...
const FlashcardViewer = memo(function FlashcardViewer({
  ...
  const handleCardAudioToggle = useCallback(() => {
    ...
    openAudioPlayer({
      ...
      title: `$<><span className="text-brand-600 dark:text-brand-400">AI</span> <><span className="text-brand-600 dark:text-brand-400">AI</span> <><span className="text-brand-600 dark:text-brand-400">AI</span> {title || 'Flashcards'}</></></> Audio`,
      autoPlay: true,
    });
