...
export default function Study() {
  ...
  return (
    ...
    >
      <AnimatePresence>
        {isDraggingToCreate && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[70] bg-violet-500/15 dark:bg-violet-400/20 backdrop-blur-sm flex items-center justify-center p-4"
          >
            <motion.div
            ...
            >
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300 max-w-md mx-auto">Release your files to start upload and processing</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="relative overflow-hidden h-28 sm:h-32 md:h-36 bg-gradient-to-br from-pink-100/90 via-fuchsia-100/80 via-30% to-orange-100/70 dark:from-cyan-900/60 dark:via-violet-900/70 dark:via-30% dark:to-blue-900/60">
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5 sm:gap-6">
              <div>
                ...
                <h2 className="text-xl sm:text-2xl xl:text-3xl font-bold text-slate-900 dark:text-white mb-2">Create New Study Set</h2>
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base sm:text-lg font-semibold text-heading mb-1">
                    Browse Files
                  </h3>
                ...
                <ArrowRight className="hidden sm:block w-5 h-5 text-slate-400 group-hover:text-slate-700 dark:group-hover:text-white transition-colors flex-shrink-0" />
              </div>
            </button>
          </div>
                ...
                {recentFiles.map((file) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left min-w-0">
                      <h3 className="font-semibold text-sm sm:text-base text-heading group-hover:text-pink-600 dark:group-hover:text-violet-400 transition-colors truncate">
                        {file.originalName}
                      </h3>
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1">
                        <FileTypeBadge mimeType={file.fileType} className="shrink-0" />
                        <span className="text-xs sm:text-sm text-muted">
                          {formatFileSize(file.fileSize)} • {formatDate(file.createdAt)}
                        </span>
                      </div>
                    </div>
                    <ArrowRight className="hidden sm:block w-5 h-5 text-slate-400 group-hover:text-pink-600 dark:group-hover:text-cyan-400 transition-colors flex-shrink-0" />
                  </button>
                ))}
              </div>
            </div>
          ...
          {files.length === 0 && (
            <div className="text-center py-10 sm:py-12">
              ...
              <p className="text-sm sm:text-base text-body mb-6 max-w-md mx-auto">
