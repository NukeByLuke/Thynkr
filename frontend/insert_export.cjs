...
const dropdownStr = `            <div className="flex items-center gap-2">
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 mt-1 hidden w-40 flex-col rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-1.5 shadow-xl group-hover:flex z-50">
                  <button 
...
const searchStr = `<span className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider">Auto-saves locally</span>`;

content = content.replace(searchStr, dropdownStr);
fs.writeFileSync(path, content);
console.log("Injected export functionality.");
